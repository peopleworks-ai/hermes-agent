...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "Loading a skill is step one, not the whole job. The skill_view result includes a "
            "linked_files manifest (references/, templates/, scripts/). When the SKILL.md routes "
            "to reference files — a 'Reference files' section, a 'How to use this skill' table, "
            "or inline mentions of references/*.md — you MUST read the reference(s) matching the "
            "work at hand with skill_view(name, file_path='references/...') BEFORE your first "
            "action, and read each one in full. SKILL.md is a router; the exact parameters, "
            "schemas, and safety boundaries live in the references. Never act from the SKILL.md "
            "summary alone when it points you to a reference.\n"
