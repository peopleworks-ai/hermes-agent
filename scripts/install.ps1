...
# The Sara fork — the desktop app pins CI SHAs from THIS repo; upstream
# NousResearch does not have them (fetching from upstream 404s every client).
$RepoUrlSsh = "git@github.com:peopleworks-ai/hermes-agent.git"
$RepoUrlHttps = "https://github.com/peopleworks-ai/hermes-agent.git"
# owner/name form for archive URLs, derived so the org lives in ONE place.
$RepoSlug = ($RepoUrlHttps -replace '^https://github\.com/', '') -replace '\.git$', ''
$PythonVersion = "3.11"
